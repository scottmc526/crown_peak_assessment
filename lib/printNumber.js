...
var myFuncs = {
  ...
  evalGroups: function(n) {
    ...
    for (var i = 0; i < arrays.length; i++) {
      if (arrays[i].charAt(0) === '0' && arrays[i].charAt(1) === '0' && arrays[i].charAt(2) === '0') {
        ...
        var newNum = arrays[i].slice(2)
        output = this.underTwenty(newNum)
        arr.push(output)
      } else if (arrays[i].charAt(0) === '0') {
  ...
  removeSpaces: function(arr) {
    ...
    for (var i = 0; i < arr.length; i++) {
      if (arr[i] === '') {
        correctedArr = arr.splice(arr[i], 1)
      }
    }
    return arr
  },
  handleLargeSum: function(n) {
    ...
    //numToPrint = this.removeSpaces(evaluatedArr)
    numToPrint = evaluatedArr.reverse().join(' ')
    return numToPrint
  },
...
console.log(myFuncs.intToEnglish(500000000000000001.00));
