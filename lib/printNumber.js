...
var bigSuffixes = [ '', 'thousand', 'million', 'billion', 'trillion', 'quadrillion']
...
var myFuncs = {
  ...
  overOneHundred: function(n) {
    ...
    //when tmpArr[i] === 2 I need bigSuffixes[i] === 1
    //when tmpArr[i] === 1 bigSuffixes[i] === 0

    for (var i = tmpArr.length -1; i >= 0; i--) {

      // console.log("TEMPARR:", tmpArr[i], 'bigsuffix:', bigSuffixes[i]);
      if (i === 0) {
        tmpArr[i] += '';
      } else {
        tmpArr[i] += ' '+ bigSuffixes[i]
      }

    }
    tmpArr = tmpArr.reverse().join(' ')
    // console.log(tmpArr);
...
console.log(myFuncs.intToEnglish(1112022333444.33));
